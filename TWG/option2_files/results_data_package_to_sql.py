    ...
    with tarfile.open(results_data_package) as tar:
        for resource in summary['resources']:                   
            if resource['path'].startswith("outputs"):
                (_, perspective, summary_set, output_file) = str.split(resource['path'], '/')
                output_type, ext = str.split(output_file, '.')
                if output_type != 'summary_info':
                    print('{}_{}_{}'.format(output_type, summary_set, perspective))
                    summary_info_file = "outputs/{}/{}/summary_info.csv".format(perspective, summary_set)
                    csv_contents = tar.extractfile(summary_info_file).read()
                    summary_info_df = pd.read_csv(io.BytesIO(csv_contents), encoding='utf8')
                    ...
                    df.to_sql('{}_{}_{}'.format(output_type, summary_set, perspective), engine)
